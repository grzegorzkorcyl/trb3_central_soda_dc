...
set Para(ModuleName) "fifo_16kx16x64"
...
#ngdbuild
set ngofile "$Para(sbp_path)/$Para(ModuleName).ngo"

set ngdbuild "$Para(FPGAPath)/ngdbuild"
set Para(result) [catch {eval exec \"$ngdbuild\" -addiobuf -dt -a $Para(arch_name) \"$ngofile\" $Para(ModuleName).ngd} msg]
#puts $msg
